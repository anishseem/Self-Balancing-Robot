...
Attached are all the related project fiels including the custom PCB gerber files, aseembly STEP files, balance code, and visual processing code.
The 3d processing, real time visualization code does not properly work in conjunction with the balancing code due to the constraints of the arduino processor. So right now, I comment out the update motor function when using it. 
